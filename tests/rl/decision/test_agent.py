"""Tests for rl.decision.agent: SeatAgent's mulligan-vs-policy dispatch routing."""
import random
import types
...
def test_random_mulligan_routes_mulligan_decision_and_records_nothing():
    ...
    from rl.decision.agent import RandomMulligan
    _decklist, vocab, state = _mulligan_decision_state()
    seat = state.active_idx
...
def test_mulligan_zero_lands_mulligans_only_a_0land_hand():
    ...
    from rl.decision.agent import MulliganZeroLands
    _decklist, vocab, state = _mulligan_decision_state()
    seat = state.active_idx
...
def _mono_red_rally_deck_ctx():
    from rl.roster import TOKEN_DEFS  # mono_red_rally mints Treasure -- needs the full token/pseudo-card set
