...
def test_build_token_set_both_seats_all_zones_and_targeting():
    ...
    # to attach to -- see rl.decision.agent._scalar_features's own self-check).
    ...
    for my_seat in (0, 1):
        ...
        # addressable for choose_stack_target -- see rl.decision.action_bridge).
...
def test_own_hand_tokenized_opponent_hidden():
    ...
    # for the ONE zone that changed (rl.model.features.build_token_set's own
    # docstring).
    _decklist_a, _decklist_b, vocab = _base_vocab()
...
def test_hand_tokens_cheap_and_full_pass_match():
    ...
    # _padded_full_mask's pointer mask is positional (rl.decision.agent's own
