...
def test_declare_blockers_gang_blocking_and_done():
    # Driven directly against a hand-built state, bypassing game.turn.
    # _declare_blockers_gen's active_idx-flip and drl_env's own eligibility
    # gate: re-opening begin_declare_blockers after each assignment is done
    # by hand here.
    ...
    # Assign Grizzly Bears to block Bear specifically (not Wolf).
    step1_done = []
    ...
    # GANG-BLOCKING: re-open and assign Panther to the SAME attacker (Bear).
    completed = []
    begin_declare_blockers(state, on_complete=lambda s: completed.append(True))
    assert completed == []
    step2_done = []
    ...
    # "Done blocking": closes a still-open resolution, no assignment required.
    completed = []
    begin_declare_blockers(state, on_complete=lambda s: completed.append(True))
...
def test_declare_blockers_no_attackers_auto_completes():
    # No attackers at all: auto-completes immediately.
    state = GameState(on_the_play=True, players=[PlayerState(True), PlayerState(False)])
    state.active_idx = 1
    completed = []
    begin_declare_blockers(state, on_complete=lambda s: completed.append(True))
...
def test_declare_blocker_assignment_extra_predicate():
    # extra_predicate applies on top of the usual "unblocked attacker"
    # filter; a plain stand-in here rather than a real keyword lookup.
