...
def test_create_token_and_blood_sac():
    ...
    try:
        ...
        # sacrificed token ceases to exist, never added to any zone
        ...
        # the draw is queued on the stack, not fired inline; nothing drawn
        # until it resolves
...
def test_blood_sac_triggers_gixian_infiltrator():
    """Sacrificing a token still fires "whenever you sacrifice another
    permanent" (Gixian Infiltrator), queued onto the stack, not applied
    immediately."""
    state = GameState(on_the_play=True)
    ...
    resolve_top_of_stack(state)  # Gixian's trigger, placed last (LIFO), resolves first
    assert gix.counters.get("+1/+1") == 1
    resolve_top_of_stack(state)  # Blood's draw
    assert len(state.hand) == 1


def test_token_limit_enforcement():
    # TOKEN_LIMIT is a shared pool across every token name, not per-name
    state = GameState(on_the_play=True)
    ...
    # no ETB trigger fires for a rejected token
    etb_calls = []
    _filler_backup = registry.EFFECT_REGISTRY[EffectId.FILLER]
