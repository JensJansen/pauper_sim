...
head. Each deck owns its OWN SetTransformer (rl.arch) as a registered child
...
features got much richer at the same time (rl.features), so the encoder is
...
Consequences worth knowing, all of them deliberate:
  ...
  - PPO recomputes the encoder forward every minibatch of every epoch
    instead of caching one frozen forward per update (rl.ppo). That is a
...
class DeckNetwork(nn.Module):
    ...
    def forward(self, mine_summary, theirs_summary, scalar_features, token_reps, pointer_token_mask,
        ...
        method calling it, because rl.agent needs token_reps and the padded
