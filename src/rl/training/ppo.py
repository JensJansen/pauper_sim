...
from rl.training.train so the multiprocessing plumbing (rl.training.rollout_parallel) and the
rollout-collection game loop (rl.training.train itself) don't have to carry this
...
def ppo_update(net, optimizer, buf, device, n_epochs=4, batch_size=64, gamma=0.99, gae_lambda=0.95,
    ...
    # rl.training.train.ent_coef_schedule and passes it in explicitly; see that
    ...
    the same tensors. Each deck now owns its encoder outright (rl.model.deck), so
    ...
    # worker serialization in rl.training.rollout_parallel -- preserves order. So the
    ...
    # The encoder is a registered child of net (rl.model.deck.DeckNetwork), so this
    ...
    for _epoch in range(n_epochs):
        ...
        for start in range(0, len(episodes), batch_size):
            ...
            # prev_action during collection (rl.decision.agent._seat_step), so the
            ...
            # deliberately (see rl.model.deck's module docstring).
