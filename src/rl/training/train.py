...
(rl.model.features + rl.model.arch + rl.model.deck + rl.decision.action_bridge) -- the piece that
actually trains a DeckNetwork. The GAE/PPO-update math lives in rl.training.ppo, and
the ProcessPoolExecutor multiprocessing plumbing for league collection lives
in rl.training.rollout_parallel; both build on the game loop (collect_rollout) and
buffer type (RolloutBuffer) defined here.

League training's own orchestration (rl.league.league_runner._run_session) drives
collect_rollout through _make_league_pairing/collect_rollout_league below,
which resamples an opponent from rl.league.league.LeaguePool every game rather than
...
keeps its own recurrent state (rl.decision.agent.SeatAgent keys hidden state by
...
not hygiene -- rl.training.ppo.ppo_update replays every episode from a ZERO state, so
...
# _raise_all_false, _is_pass) live in rl.decision.agent -- collect_rollout drives them
...
def collect_rollout(pairing, n_games, horizon, rng, device="cpu", record=True, greedy=False, game_logs=None,
    ...
    (rl.decision.agent). This function owns only ATTRIBUTION (bandit mulligan reward +
    ...
    league-specific use (rl.league.league.LeaguePool PFSP stat updates, see
    ...
    try:
        for _ in range(n_games):
            ...
            # one place this happens -- see rl.decision.agent.SeatAgent.reset for why it
...
def collect_rollout_league(training_deck_name, live_nets, mulligan_nets, deck_ctxs, decklists_by_name,
    ...
    bucketing rl.league.league_runner._run_eval_vs_history/_run_eval_vs_gauntlet already
    ...
    in rl.league.league_runner._run_session) gets updated once, after collection, regardless
    ...
    rl.league.league.LeaguePool.sample_opponent's own docstring."""
    choice_sink = {}
    outcomes = []
...
def batch_size_for_iteration(cumulative_games, horizon_games=50_000, start=4, cap=16, n_steps=6):
    ...
    policy's unit of sampling -- see rl.training.ppo.ppo_update; it counted transitions
