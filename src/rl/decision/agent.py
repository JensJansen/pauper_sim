...
mask/feature builders it needs), keeping them out of rl.training.train so that rl.training.train
can import SeatAgent without a circular dependency (rl.decision.agent depends on
nothing in rl.training.train)."""
...
def _scalar_features(state, seat_idx, horizon):
    ...
    rl.model.deck.SCALAR_FEATURE_DIM documents (mana-pool cap of 8, matched here).
    ...
    CONTENTS hidden (rl.model.features.build_token_set's own docstring). My own
    ...
    def _read(s):
        ...
        # (rl.model.mulligan.MulliganNet.N_SCALAR) while the main policy did not,
...
def _hand_cost_reduction_deltas(state, seat_idx):
    ...
    docstring) -- rl.model.features._token_row's cost_reduction_delta slot for a
    ...
    shared/interned objects -- see rl.model.features.build_token_set's own
...
def _resolve_pointer_identity(state, obj):
    """A pointer candidate's identity object (see rl.model.features.build_token_set's
...
def _seat_step(state, seat, deck_ctx, net, horizon, device, greedy=False, hidden=None, prev_action=None):
    ...
    hidden: this seat's recurrent state (rl.model.deck.DeckNetwork's GRU), threaded
...
class SeatAgent:
    ...
    def reset(self):
        ...
        game, for every agent -- rl.training.train.collect_rollout's own game loop is
        the one place that does it.

        Not optional bookkeeping. A SeatAgent is routinely REUSED across
        games: rl.league.league.LeaguePool.load_snapshot_agent caches by path and
    ...
    def decide(self, state, seat, horizon, device, greedy=False):
        ...
        # exact object on BOTH seats (rl.training.train.collect_rollout_league:
...
# rl/decision/heuristic_agent.py as of 2026-08-17. It imports the decision plumbing above
