...
# advantage spread is almost pure noise, and rl.training.ppo's unconditional
...
class LeaguePool:
    ...
    def save_opponent_stats(self):
        ...
        rl.league.league_runner._run_session on the SAME cadence as its other checkpoints
    ...
    def record_outcome(self, training_deck_name, opponent_key, won):
        ...
        rl.training.train.collect_rollout(_league(_parallel)) -- rides along with
    ...
    def register_snapshot(self, deck_name, net, mulligan_net=None):
        ...
        while len(self.snapshots[deck_name]) > self.max_snapshots_per_deck:
            ...
            # older selves once they aged out -- see rl.league.league_runner._run_eval_vs_history,
    ...
    def load_snapshot_agent(self, snapshot_path, deck_ctx):
        ...
        to be cleared between them. rl.training.train.collect_rollout's own game loop
        ...
        (rl.model.deck.DeckNetwork registers it). A fresh SetTransformer is built
        ...
        the caller had to supply one (rl.training.train even passed the TRAINING deck's
