...
PPO_DEFAULTS = {
    ...
    "target_kl": 0.03,
    "n_epochs": 4,
    # Floor on the advantage-normalization divisor (rl.training.ppo.ppo_update). 0.0 is
    ...
    "adv_norm_floor": 0.0,
    # Entropy bonus. None = use rl.training.train.ent_coef_schedule's 0.02 -> 0.005
...
def build_deck_net(vocab_size, n_actions, trunk_hidden=None):
    """One deck's full network: its OWN perception encoder (rl.model.arch) plus the
...
def _run_session(n_iterations, games_per_iteration, snapshot_every, executor, n_workers,
    ...
    # 22,848-games/deck checkpoint (analysis/eval/bench_gpu_vs_cpu.py): updates are
    ...
    # -- the horizon batch_size_for_iteration and ent_coef_schedule (rl.training.train)
    ...
    # Per-deck mulligan model (rl.model.mulligan): owns the pregame keep/mulligan +
    ...
    for name in deck_names:
        ...
        # CPU -- leaving the module split across two devices. rl.model.mulligan.update
    ...
    for iteration in range(n_iterations):
        ...
        # ent_coef_schedule's own docstrings (rl.training.train) for why session-local
        ...
        # rl.training.rollout_parallel: these cross a process boundary to CPU-only
...
def _run_eval(eval_decks, games_per_pairing, greedy, seed, game_logs, matchup=None,
    ...
    (argmax) by default, matching rl.decision.agent/rl.model.mulligan's own "greedy=True is
...
def _run_eval_vs_history(deck_name, live_net, mulligan_net, deck_ctx, decklist, league_dir,
    ...
    "game_over" event (see rl.training.train) to score winners -- greedy=True so this
...
def _run_eval_vs_heuristic(deck_name, live_net, mulligan_net, deck_ctx, decklist, horizon,
    ...
    """Plays deck_name's CURRENT live net against a HeuristicAgent (rl.decision.heuristic_agent)
