...
into league training (whose rl.mulligan module default was still cost=0.02)
...
sees at every in-game decision -- rl.features.build_token_set's full per-card
...
sharing mechanics. Card indices come from rl.features.CardVocab.
"""
...
class MulliganNet(nn.Module):
    ...
    hand's full structured token set (rl.features.build_token_set -- the SAME
    ...
    pointer mechanism rl.deck.DeckNetwork uses for its own targeting head,
...
def decide(net, vocab, state, seat, record, greedy=False):
    ...
    Builds the SAME per-card token set (rl.features.build_token_set) and runs
