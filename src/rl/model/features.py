...
class CardVocab:
    ...
    def __init__(self, decklists, token_card_defs=(), vocab_path=None):
        ...
        rl.roster). Append-only: an index a persisted file already gave
...
def _token_row(name, zone, is_mine, vocab, permanent=None, owner_idx=None, enchanting_auras=None, state=None,
    ...
    rl.decision.agent._hand_cost_reduction_deltas) -- zero for every other token.
...
def _stack_target_map(state):
    ...
    one) -- rl.decision.agent._scalar_features reads this half directly instead.
...
def build_token_set(state, my_seat_idx, vocab, include_rows=True, hand_cost_reduction=None):
    ...
    blocked-by set construction) is dropped. For rl.decision.agent._build_decision's
    ...
    _token_row) for MY hand only -- rl.decision.agent._hand_cost_reduction_deltas
    ...
    named hand cards by object identity in rl.decision.action_bridge.pointer_legal_
    mask's id()-keyed _ID_MATCHED_KINDS branch). The pointer-network action
    head (rl.model.deck) matches a legal target back to "which row of this token
    ...
    an unhashable dict, see rl.decision.action_bridge -- so two same-named graveyard
    ...
    bit on; rl.decision.agent._scalar_features surfaces it as a scalar instead. My own
    ...
    surfaced instead as a scalar, not a token -- rl.decision.agent._scalar_features,
    ...
    obj_controllers, _player_controllers = _stack_target_map(state) if include_rows else ({}, None)  # player half is rl.decision.agent._scalar_features's job; skip entirely when rows aren't needed
