...
def begin_pay_cost(state, cost, on_complete):
    ...
    # The STRANDING INVARIANT, checked at the one place it can still be
    ...
    # the guilty call site directly in the traceback. An explicit raise, not
    # `assert` -- this is the single most load-bearing self-check in the
    # payment path, and `assert` silently no-ops under `python -O`/
    # PYTHONOPTIMIZE, which nothing here uses today but which would turn
    # this into exactly the opaque all-False-mask failure it exists to
    # prevent, with the guilty call site gone from the traceback.
