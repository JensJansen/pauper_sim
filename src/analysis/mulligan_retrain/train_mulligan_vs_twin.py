...
policy. rl/model/mulligan.py's 2026-08-20 fix gave the net eyes (it now reads the
...
--entropy-coef overrides rl.model.mulligan.ENTROPY_COEF for this run only (the
...
rl.model.mulligan's own per-mulligan-count reward penalty (MULLIGAN_COST) --
...
rl.model.mulligan's module default was still cost=0.02 -- attaching a checkpoint
...
it the moment training resumed. Removed from rl.model.mulligan.mulligan_reward
entirely rather than leave that mismatch in place. See rl/model/mulligan.py's
own docstring.
...
from rl.model import mulligan as mulligan_mod  # noqa: E402 -- module itself, so --entropy-coef can patch its ENTROPY_COEF global
...
def _train_mulligan(main_net, main_decklist, main_bucket, deck_ctx,
    ...
    Reward is filled in by collect_rollout itself (rl.model.mulligan.mulligan_
