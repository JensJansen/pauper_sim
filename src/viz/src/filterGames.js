...
export function filterGames(games, criteria = {}) {
  ...
  // now filters on scores[0] (score 1, the training reward, always
  // present) rather than a display-only "score2" that no longer has a
  // guaranteed scale (a deck's own scoring_fns could be anything, e.g.
  // Tron's tron_online_score is a plain 0/1, not a 0-100 percentage).
  ...
  return games.filter((g) => {
    ...
    if (scoreMin != null && g.scores[0] < scoreMin) return false;
    if (scoreMax != null && g.scores[0] > scoreMax) return false;
